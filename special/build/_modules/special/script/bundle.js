...
var $ext = {
  filterText: function filterText(data) {
    ...
    // phpタグを取得
    var php = data.match(/<\?php([\s\S]*?) \?>/g);
    // scriptタグを取得
    var script = data.match(/<script(?: .+?)?>[\s\S]*?<\/script>/g);
    ...
    for (var _i in script) {
      str = str.replace(script[_i], "");
    }

    this.createdMain(str, script.join(''));
  },
  createdMain: function createdMain(data, script) {
    ...
    iframe.onload = function (e) {
      var that = e.target;
      var ifm = that.contentWindow || that.contentDocument.document || that.contentDocument;
      var head = ifm.document.getElementsByTagName('head')[0].innerHTML;
      var main = ifm.document.getElementById('str-main').innerHTML;
      var result = '<div id="str-main">' + main + '</div>';
