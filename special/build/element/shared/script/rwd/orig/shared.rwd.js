      ...
      if(this._swipe && this._supportTouch){
        this._slideList.on({
          ...
          'touchstart': function(e) {
            ...
            that._touchYstart = event.changedTouches[0].screenY;
          ...
          'touchmove': function(e) {
            ...
            // 縦か横か判定
            var moveY = that._touchYstart - event.changedTouches[0].screenY;
            if (moveY <= 5 && moveY >= -5 && !that._isMoveX && !that._isMoveY) {
                that._isMoveX = true;
            } else if ((moveY > 5 || moveY < -5) && !that._isMoveX && !that._isMoveY) {
                that._isMoveY = true;
            }
          ...
          'touchend': function(e) {
            ...
            // 基準にする移動距離
            var _grid = that._width * 0.2;

            // 移動距離が基準値を超えた場合はスライドを移動させる
            if((((new Date()).getTime() - that.startTime <= 300) && that._isMoveX) || _dist > _grid){
...
$.special.opinionForm = function() {
  ...
  $('#Act_opinion_post_sp, .Act_opinion_post_sp').on('click', function() {
    ...
    window.open(s_url_send + '?url=' +
      ...
      encodeURIComponent(location.hash), '_blank');
...
$.fixedMoveToTop = function() {
  // トップに戻るボタンが無い場合は挿入する
  if ($('#Act_MovePageTop').length <= 0 && $('.str-footer').length <= 0) {
...
$.headerButtons = function() {
  ...
  o_history_button.length > 0 && o_history_button.on('click', function() {
    $.travelko_history.checkAndMoveToHistory();
  });
  // よくある質問ボタン
  var o_faq_button = $('.Act_faq_button');
  o_faq_button.length > 0 && o_faq_button.on('click', (function() {
    var a_menu = ['w_hotel', 'w_tour', 'w_air', 'w_dp', 'w_optional', 'w_wifi', 'j_hotel', 'j_tour', 'j_air', 'j_dp', 'j_optional', 'j_bus', 'j_bustour', 'j_rentacar', 'j_traindp']
